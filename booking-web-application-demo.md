...
Node.js เป็น JavaScript runtime ที่ทำงานนอกเบราว์เซอร์ ส่วน Express เป็น framework สำหรับสร้าง web application และ API:
...
2. Express Middleware:
   - body-parser: แปลงข้อมูล request
   - cors: จัดการ Cross-Origin Resource Sharing
   - express.json(): แปลง JSON

## 3. SQLite

SQLite เป็นฐานข้อมูลแบบ file-based ที่ไม่ต้องมี server:

1. ข้อดี:

   - ติดตั้งง่าย
   - ไม่ต้องมี server
   - เหมาะกับแอพขนาดเล็กถึงกลาง

2. การใช้งานกับ Node.js:
   - ใช้ผ่าน sqlite3 module
   - รองรับ async/await
...
3. Web Browser (Chrome หรือ Firefox)
4. Postman
...
8. สร้าง request เพื่อ Delete Booking ระบุ Url คือ DELETE: {{baseUrl}}/api/bookings/1
   - Headers: Authorization: Bearer {{token}}
...
8.1 ปรับปรุงแก้ไขโค้ดเพื่อให้แสดงผลการลบเป็น status : "ลบข้อมูลสำเร็จโดย [ชื่อนักศึกษา]"
ดังรูป
...
### ไฟล์ 3.2.8 การสร้าง AdminDashboard Component
...
### ส่วนที่ 5: การจัดการข้อมูลการจอง

#### 5.1 การปรับปรุงที่ต้องทำในส่วนก่อนหน้า

1. ปรับปรุง Backend API (server.js):

   - เพิ่ม endpoint สำหรับดึงข้อมูลการจองรายเดือน
   - เพิ่ม endpoint สำหรับดึงสถิติการจอง
   - แก้ไข endpoint การอัพเดตให้รองรับการอัพเดตสถานะ

2. ปรับปรุงฐานข้อมูล (database.js):
   - เพิ่มฟิลด์ status ในตาราง bookings
   - เพิ่มฟิลด์ comment สำหรับบันทึกหมายเหตุ

```javascript
// ปรับปรุงโครงสร้างตาราง bookings
db.run(`
...
#### 5.2 การสร้างหน้าจัดการข้อมูลการจอง

1. สร้างไฟล์ `src/components/BookingManagement.jsx`:
...
const BookingManagement = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("คุณต้องการลบข้อมูลการจองนี้ใช่หรือไม่?")) {
      try {
        ...
        setError("ไม่สามารถลบข้อมูลการจองได้");
      }
    }
  };
  ...
  return (
    <div className="container mx-auto p-4">
      <h2 className="text-2xl font-bold mb-4">จัดการข้อมูลการจอง</h2>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg w-full max-w-md">
            ...
            <div className="space-y-4">
              ...
              {/* เพิ่มฟิลด์อื่นๆ ที่ต้องการแก้ไข */}
        ...
        <table className="min-w-full bg-white">
          <thead>
            <tr>
              ...
              <th className="py-2 px-4 border-b">วันที่เช็คอิน</th>
              <th className="py-2 px-4 border-b">วันที่เช็คเอาท์</th>
              <th className="py-2 px-4 border-b">ประเภทห้อง</th>
              <th className="py-2 px-4 border-b">จำนวนผู้เข้าพัก</th>
...
#### 5.3 การปรับปรุงเส้นทางการนำทาง
...
#### 5.4 การทดสอบการจัดการข้อมูล

1. ทดสอบการแสดงข้อมูล:

   - เข้าสู่ระบบด้วยบัญชีผู้ดูแล
   - เข้าไปที่หน้าจัดการข้อมูลการจอง
   - ตรวจสอบการแสดงข้อมูลในตาราง

2. ทดสอบการแก้ไขข้อมูล:

   - คลิกปุ่มแก้ไขที่รายการใดรายการหนึ่ง
   - แก้ไขข้อมูลในฟอร์ม
   - บันทึกการเปลี่ยนแปลง
   - ตรวจสอบการอัพเดตในตาราง

3. ทดสอบการลบข้อมูล:
   - คลิกปุ่มลบที่รายการใดรายการหนึ่ง
   - ยืนยันการลบข้อมูล
   - ตรวจสอบการลบออกจากตาราง

#### 5.5 การบันทึกผลการทดสอบ

| การทดสอบ      | ผลที่คาดหวัง                  | ผลลัพธ์ | หมายเหตุ |
| ------------- | -------------------------- | ----- | ------- |
| แสดงข้อมูล      | แสดงข้อมูลการจองทั้งหมดในตาราง |       |         |
| แก้ไขข้อมูล      | แก้ไขและบันทึกข้อมูลสำเร็จ        |       |         |
| ลบข้อมูล        | ลบข้อมูลสำเร็จ                 |       |         |
| การตรวจสอบสิทธิ์ | เฉพาะผู้ดูแลเข้าถึงได้           |       |         |
